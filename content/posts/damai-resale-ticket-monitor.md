...
经过验证 headers 是统一的不会变也没有加密参数，cookie 也是一致的但有未知参数 `_m_h5_tk`和 `_m_h5_tk_enc`
...
其中 `data` 显而易见，场次接口的 `itemId` 就是这个演出的 id，其他参数固定；座次接口的 `itemId` 是演出 id，`dataId`就是当前场次的 id（例如 10.21 场），其他参数固定。
接着我们可以打断点看一下这个 `t` 和 `sign` 是如何生成的。F12 调出控制台，下一个 xhr 断点，可以直接打到 `sign`上，也可以打到对应链接上。
...
可以看到这个函数就是将 `token`、`c`、`s`、`data`，以&进行分隔拼接，然后再进行加密得到 `sign`。
...
<img src="https://drive.404fix.cn/i/SnVNxSm1.png" width="200" alt="大麦" />